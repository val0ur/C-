...
df1 <- df1[with(df1, order(time)), ]   # could be different for each csv 
head(df1, 20)
...
df2 <- df2[with(df2, order(time)), ]   # could be different for each csv 
head(df2, 20)
...
# add status column for coloring each event | change status ex(timeline, install_program, search_terms)
df3$status <- "timeline"
df3 <- df3[with(df3, order(time)), ]   # could be different for each csv 
head(df3, 20)
...
# gather 2 DF into 1 DF | df1 + df2 => df
